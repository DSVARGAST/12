...
function DashboardHome({ publications, users }) {
  const dashboardData = useMemo(() => {
    ...
    const weekdayTotals = weekdayLabels.map((label, index) => ({
      label,
      value: 0,
      dayIndex: index,
    }));
    ...
    const platformOrder = Object.keys(platformPalette);
    const platformTotals = platformOrder.reduce((acc, platform) => {
      acc[platform] = 0;
      return acc;
    }, {});
    ...
    const platformTotalValue = Object.values(totals.platformTotals).reduce((acc, value) => acc + value, 0) || 1;
    const platformData = platformOrder.map((platform) => ({
      name: platform,
      value: Math.round((totals.platformTotals[platform] / platformTotalValue) * 100),
      color: platformPalette[platform],
    }));
